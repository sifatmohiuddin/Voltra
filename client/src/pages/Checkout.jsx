...
const PAYMENT_OPTIONS = [
  ...
  },
  {
    ...
    label: 'bKash',
    icon: Smartphone,
    hint: "You'll approve the payment from your bKash app",
  },
  {
    value: 'Nagad',
    label: 'Nagad',
    icon: Smartphone,
    hint: "You'll approve the payment from your Nagad app",
  },
];
...
export default function Checkout() {
  ...
  // Checkout starting is a real signal of purchase intent.
  // This is where an abandoned-cart event gets logged,
  // not on every add-to-cart.
  useEffect(() => {
      ...
      const order = await api.placeOrder({
        items: items.map((i) => ({
          ...
          variantCombinationId:
            i.variantCombinationId || undefined,
          quantity: i.quantity,
        })),
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
      ...
      <form
        ...
        className="grid md:grid-cols-[1fr,320px] gap-8"
      >
          ...
          <section>
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              {['Inside Dhaka', 'Outside Dhaka'].map((zone) => (
                <label
                  key={zone}
                  className={`flex items-center gap-3 rounded-xl border p-4 cursor-pointer transition-colors ${form.deliveryZone === zone
          ...
          <section>
            ...
            <div className="space-y-2.5">
              {PAYMENT_OPTIONS.map((opt) => (
                <label
                  key={opt.value}
                  className={`flex items-center gap-3 rounded-xl border p-4 cursor-pointer transition-colors ${form.paymentMethod === opt.value
                      ? 'border-volt bg-volt-soft'
                      : 'border-line hover:border-ink-faint'
                    }`}
                >
        ...
        <div className="h-fit rounded-2xl border border-line bg-panel p-6 space-y-4">
          ...
          <div className="space-y-4 max-h-80 overflow-y-auto pr-1">
            {items.map((item) => (
              ...
              >
                {/* Product image */}
                <div className="w-14 h-14 rounded-lg overflow-hidden bg-paper border border-line shrink-0">
                  <img
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <button
                      ...
                      className="text-ink-faint hover:text-stock-out transition-colors"
                ...
                {/* Item total */}
                <span className="font-mono font-medium text-ink text-xs shrink-0">
          ...
          {coupon ? (
            <div className="flex items-center justify-between rounded-lg bg-volt-soft px-3 py-2">
              ...
              <button
                ...
                className="text-volt-dim hover:text-ink"
            ...
            <div>
              <div className="flex gap-2">
                ...
                <button
                  ...
                  disabled={
                    checkingCoupon || !couponInput.trim()
                  }
              ...
              {couponError && (
                <p className="text-xs text-stock-out mt-1.5">
                  {couponError}
                </p>
              )}
            </div>
          ...
          {/* Error */}
          {error && (
            <p className="text-sm text-stock-out">
              {error}
            </p>
          )}
